...
public class InfixCalculator2 implements InfixCalculatorInterface {
    ...
    // note that "^" here is the exponentiation operator, not a bitwise
    // exclusive OR
    // also note that the sine and cosine functions in this program take units
    // of randians, not degrees
            ...
            while ((line = bufferedReader.readLine()) != null) {
                ...
                for (int i = 0; i < tokens.length; i++) {
                    ...
                    for (int j = 0; j < parsedResult.length; j++) {
                        // if operators appeared
                        if (precedenceOrder(parsedResult[j]) >= 0) {
                            parseIndicator = 1;
                        }
                    }

                    // case 1:
                    // if an operator is mixed in the string
                    if (parseIndicator == 1) {
                        // declare critical vairables
                        int parseNumberIndicator = 0;
                        String ParsedNumberString;

                        // check each element of the mixed string
                        for (int j = 0; j < parsedResult.length; j++) {
                            // if an operator appeared
                            if (precedenceOrder(parsedResult[j]) >= 0) {

                                if (parseNumberIndicator == 1) {
                                    // assemble the number back

                                    ParsedNumberString = sb.toString();

                                    // System.out.println("assembled string is
                                    // ." + ParsedNumberString);
                                    sb.setLength(0);
                                    // add ParsedNumberString to arrayList

                                    arrayList.add(ParsedNumberString);

                                }

                                // add the operator to arrayList

                                arrayList.add(parsedResult[j]);
                            }

                            // if the parsed elements are non-operators, i.e.
                            // possibly from numbers
                            // e.g. 11 is parsed into 1 and 1, we need to
                            // assemble the number back
                            else {
                                parseNumberIndicator = 1;

                                sb.append(parsedResult[j]);
                            }
                        }
                        // when reach the end of the parsed mixed string
                        ParsedNumberString = sb.toString();
                    ...
                    // case 2:
                    // if no operator is mixed in the string,
                    // add the unparsed result to the array list
                    if (parseIndicator == 0) {
    ...
    public static void main(String[] args) {
        InfixCalculator2 testInstance = new InfixCalculator2();
        if (args.length >= 0) {

            arrayListGlobal.clear();

            // read the file
            testInstance.readFile("test.txt");
            ...
            // write the file
            testInstance.writeFile("output.txt", elementArraySizeGlobal);
            System.out.println("Text File named 'output.txt' is created.");
        }
    }
}
