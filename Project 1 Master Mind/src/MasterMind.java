...
public class MasterMind implements MasterMindInterface {
	...
	public void newGame() {
		...
		while (true){
			System.out.println("From 'RED', 'GREEN', 'BLUE', 'YELLOW', 'ORANGE', 'PINK'"
					+ "\nHow many colors does your token have? (1-6)");
			Scanner scannerOption = new Scanner(System.in);
			int Option = scannerOption.nextInt();
			if (Option > 0 && Option < 7) {
				System.out.printf("OK. %d colors.\n", Option);
				System.out.println("Note that the colors guessed by the program can repeat.");
				colors = Option;
				break;
			} else {
		...
		while (true){
			System.out.println("How many positions does your token have? (1-6)?");
		...
		// reset the linked list data structure for new game
		headNode = null;
		
		// reset the reducedPossibleColors for new game
		reducedPossibleColors = new ArrayList<String>(Arrays.asList(possibleColors));

		// reset userMistakeIndicator for new game
		userMistakeIndicator = 0;

		// reset initialGuessIndicator for new game
		...
		if (positions == 1){
			for (int i = 0; i < n; i++) {
	                ...
	                arrayList.add(possibleColors[i]);
		...
		if (positions == 2){
			for (int i = 0; i < n; i++) {
	            for (int j = 0; j < n; j++) {
	                ArrayList<String> arrayList = new ArrayList<String>();
	                arrayList.add(possibleColors[i]);
	                arrayList.add(possibleColors[j]);
			...
			for (int i = 0; i < n; i++) {
	            for (int j = 0; j < n; j++) {
	                for (int k = 0; k < n; k++) {
                        ...
                        arrayList.add(possibleColors[i]);
                        arrayList.add(possibleColors[j]);
                        arrayList.add(possibleColors[k]);
	            ...
	            for (int j = 0; j < n; j++) {
	                for (int k = 0; k < n; k++) {
	                    for (int l = 0; l < n; l++) {
	                        ...
	                        arrayList.add(possibleColors[j]);
	                        arrayList.add(possibleColors[k]);
	                        arrayList.add(possibleColors[l]);
					...
					for (int k = 0; k < n; k++) {
						for (int l = 0; l < n; l++) {
							for (int m = 0; m < n; m++) {
			                        ...
			                        arrayList.add(possibleColors[k]);
			                        arrayList.add(possibleColors[l]);
			                        arrayList.add(possibleColors[m]);
						...
						for (int l = 0; l < n; l++) {
							for (int m = 0; m < n; m++) {
								for (int o = 0; o < n; o++) {
			                        ...
			                        arrayList.add(possibleColors[l]);
			                        arrayList.add(possibleColors[m]);
			                        arrayList.add(possibleColors[o]);
	...
	// indicate whether to delete a possibility form the linked list
	...
	public static void main(String[] args) {
		...
		MasterMind testInstance = new MasterMind();
