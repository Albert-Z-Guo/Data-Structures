...
public class Hash implements Hashing {
	...
	public Hash() {
		hashTableArray = new String[20996011];
	};
	...
	public boolean lookup(String x) {
		...
		if (hashTableArray[hashValue] == null)
			return false;
		else if (hashTableArray[hashValue].equalsIgnoreCase(x) == true)
			return true;
		else{
			// searching forward
			for (int i = hashValue + 1; i < hashTableArray.length; i++) {
				if (hashTableArray[i] == null)
					nullOccurance++;
				if (hashTableArray[i] != null && hashTableArray[i].equalsIgnoreCase(x))
					return true;
				if (nullOccurance == 2)
					return false;
			}
	...
	public void linearProbing(String itemToBeInserted, String[] arrayPerformedOn) {
	...
	public void insert(String item) {
		...
		if (lookup(item) == false) {
			...
			if (loadFactor > 0.5) {
				...
				String[] enlargedHashTableArray = new String[size() * 2];
	...
	public void print() {
		for (int i = 0; i < hashTableArray.length; i++) {
			...
			if (hashTableArray[i] != null) {
				System.out.println(i + ") " + hashTableArray[i]);
			}
		}
	}
}
