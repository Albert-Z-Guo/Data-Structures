...
public class IntersectionHash implements IntersectionHashing {

	// declare hashTableArray as a string array to store data
	Intersection[] hashTableArray;
	...
	public IntersectionHash() {
		hashTableArray = new Intersection[20996011];
	};
	...
	public boolean lookup(Intersection x) {
		int nullOccurance = 0;
		int hashValue = hash(x.intersectionID, size());
		if (hashTableArray[hashValue] == null)
			return false;
		else if (hashTableArray[hashValue].intersectionID.equalsIgnoreCase(x.intersectionID) == true)
			return true;
		else{
			// searching forward
			for (int i = hashValue + 1; i < hashTableArray.length; i++) {
				if (hashTableArray[i] == null)
					nullOccurance++;
				if (hashTableArray[i] != null && hashTableArray[i].intersectionID.equalsIgnoreCase(x.intersectionID))
					return true;
				if (nullOccurance == 2)
					return false;
			}
	...
	public void linearProbing(Intersection itemToBeInserted, Intersection[] arrayPerformedOn) {
		...
		// declare hash key
		int i = hash(itemToBeInserted.intersectionID, size());
	...
	public void insert(Intersection item) {
		...
		if (lookup(item) == false) {
			...
			if (loadFactor > 0.5) {
				...
				Intersection[] enlargedHashTableArray = new Intersection[size() * 2];
				...
				int hashValue = hash(item.intersectionID, size());
	...
	//method to retrieve the Intersection object
	public Intersection retrieve(String vertex){
		return hashTableArray[hash(vertex, size())];
	}
	...
	public void print() {
		for (int i = 0; i < hashTableArray.length; i++) {
			...
			if (hashTableArray[i] != null) {
				System.out.println(i + ") " + hashTableArray[i].intersectionID);
			}
		}
	}
}
